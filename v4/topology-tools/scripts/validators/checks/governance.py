...
def check_l0_contracts(
    ...
    *,
    ...
    if isinstance(created, str) and isinstance(last_updated, str):
        try:
            ...
            if updated_dt < created_dt:
                errors.append(f"L0_meta.metadata.last_updated '{last_updated}' is earlier than created '{created}'")
        except ValueError:
            warnings.append("L0_meta.metadata.created/last_updated should use YYYY-MM-DD format")
    ...
    if version and changelog:
        ...
        if not has_version:
            warnings.append(f"L0_meta.metadata.changelog does not contain current version '{version}'")

    default_sec_ref = refs_defaults.get("security_policy_ref")
    if default_sec_ref and default_sec_ref not in ids["security_policies"]:
        errors.append(f"L0_meta.defaults.refs.security_policy_ref '{default_sec_ref}' does not exist")

    default_mgr_ref = refs_defaults.get("network_manager_device_ref")
    if default_mgr_ref:
        l1_devices = topology.get("L1_foundation", {}).get("devices", []) or []
        device_map = {d.get("id"): d for d in l1_devices if isinstance(d, dict) and d.get("id")}
        if default_mgr_ref not in ids["devices"]:
            errors.append(f"L0_meta.defaults.refs.network_manager_device_ref '{default_mgr_ref}' does not exist")
        else:
