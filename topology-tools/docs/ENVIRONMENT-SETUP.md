...
```bash
./scripts/environment/setup-dev-environment.sh
```

Windows (PowerShell):

```powershell
./scripts/environment/setup-dev-environment.ps1
```
...
```bash
TASK_VERSION=3.45.4 ./scripts/environment/setup-dev-environment.sh
```

Если пакет `go-task` отсутствует в репозитории, используйте `./scripts/environment/setup-dev-environment.sh`.
...
```bash
./scripts/terraform/generate-tfvars.sh proxmox
```

```powershell
./scripts/terraform/generate-tfvars.ps1 proxmox
```
...
```powershell
python topology-tools/compile-topology.py --secrets-mode passthrough
...
- `task validate:v5` использует `inject` по умолчанию (через `scripts/orchestration/lane.py`).
