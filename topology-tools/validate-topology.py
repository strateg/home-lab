...
class SchemaValidator:
    ...
    def _check_file_placement(self) -> None:
        ...
        for file_path in topology_dir.rglob('*.yaml'):
            ...
            if (
                ...
                self._check_expected_prefix(
                    rel,
                    self._policy_get(['paths', 'l1_media_root'], "topology/L1-foundation/media/"),
                    f"topology/L1-foundation/media/{obj.get('id', file_path.name)}.yaml"
                )
                continue
            ...
            if (
                ...
                self._check_expected_prefix(
                    rel,
                    self._policy_get(['paths', 'l1_media_attachments_root'], "topology/L1-foundation/media-attachments/"),
                    f"topology/L1-foundation/media-attachments/{obj.get('id', file_path.name)}.yaml"
                )
                continue
    ...
    def _check_device_taxonomy(self, ids: Dict[str, Set[str]]) -> None:
        ...
        storage_ctx = self._l1_storage_context()
        power_device_ids = {
        ...
        for device in devices:
            ...
            self._check_device_storage_taxonomy(device, storage_ctx=storage_ctx)

        self._check_l1_media_inventory(ids, storage_ctx=storage_ctx)
    ...
    @staticmethod
    def _storage_disk_port_compatibility() -> Dict[str, Set[str]]:
        return {
    ...
    @staticmethod
    def _storage_mount_port_compatibility() -> Dict[str, Set[str]]:
        return {
    ...
    def _l1_storage_context(self) -> Dict[str, Any]:
    ...
    def _normalize_device_storage_inventory(self, device: Dict[str, Any], storage_ctx: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _check_device_storage_taxonomy(self, device: Dict[str, Any], storage_ctx: Optional[Dict[str, Any]] = None) -> None:
        """Validate L1 compute storage slots and resolved media taxonomy."""
        dev_id = device.get('id', 'unknown')
        dev_class = device.get('class')
        dev_substrate = device.get('substrate')

        inventory = self._normalize_device_storage_inventory(device, storage_ctx=storage_ctx)
        ...
        disk_port_compat = self._storage_disk_port_compatibility()
        mount_port_compat = self._storage_mount_port_compatibility()
    ...
    def _check_l1_media_inventory(self, ids: Dict[str, Set[str]], storage_ctx: Optional[Dict[str, Any]] = None) -> None:
        ...
        for media in media_registry:
            ...
            if media_id in seen_media_ids:
                self.errors.append(f"L1 media_registry: duplicate media id '{media_id}'")
