    ...
    *,
    ...
    for composition_file, expected_lines in expected_lines_by_file.items():
        ...
        for expected_line in expected_lines:
            if expected_line not in content:
                errors.append(f"Include contract: '{composition_file}' must define `{expected_line}`")
    ...
    for domain_dir in migrated_dirs:
        if not domain_dir.exists():
            continue
        index_files = sorted(candidate.relative_to(root).as_posix() for candidate in domain_dir.rglob("_index.yaml"))
        for index_file in index_files:
            errors.append(f"Include contract: manual index file is not allowed in migrated domain ('{index_file}')")
    ...
    for workload_root in workload_roots:
        ...
        for file_path in sorted(workload_root.rglob("*.yaml")):
            ...
            if has_alias:
                ...
                errors.append(f"Include contract: YAML aliases are not allowed in modular L4 workloads ('{rel}')")
...
def _check_device_file_path(
    *,
    ...
    expected_group = policy_get(["l1_device_group_by_substrate", str(substrate)], "owned")
    ...
    if not rel.startswith(devices_root):
        ...
        emit_by_severity(
            severity,
            f"File placement lint: device file '{rel}' should be in L1 devices " f"(suggested: '{expected_path}')",
        )
        return
    ...
    *,
    ...
    for file_path in topology_dir.rglob("*.yaml"):
        ...
        if isinstance(obj_id, str) and obj_id and file_path.stem != obj_id:
            severity = policy_get(["checks", "file_placement", "filename_id_mismatch_severity"], "warning")
            emit_by_severity(severity, f"File '{rel}': filename '{file_path.stem}' differs from id '{obj_id}'")
        ...
        if {"id", "endpoint_a", "endpoint_b", "mode"}.issubset(obj.keys()) and str(obj.get("id", "")).startswith(
            "plink-"
        ):
            _check_expected_prefix(
                rel=rel,
        ...
        if (
            ...
            _check_expected_prefix(
                rel=rel,
                expected_prefix=policy_get(
                    ["paths", "l1_media_attachments_root"], "topology/L1-foundation/media-attachments/"
                ),
        ...
        if isinstance(obj.get("id"), str) and obj.get("id", "").startswith("fw-") and "chain" in obj:
            _check_expected_prefix(
                rel=rel,
                expected_prefix=policy_get(
                    ["paths", "l2_firewall_policies_root"], "topology/L2-network/firewall/policies/"
                ),
...
def check_device_taxonomy(
    ...
    *,
    ...
    power_device_ids = {
        d.get("id") for d in devices if isinstance(d, dict) and d.get("id") and d.get("class") == "power"
    }
    class_type_map = {
    ...
    for device in devices:
        ...
        if allowed_types and dev_type not in allowed_types:
            errors.append(f"Device '{dev_id}': class '{dev_class}' is inconsistent with type '{dev_type}'")

        if dev_type == "cloud-vm" and location_ref in locations and location_type != "cloud":
            errors.append(f"Device '{dev_id}': cloud-vm is expected in cloud location, got '{location_ref}'")

        if dev_type == "cloud-vm" and dev_substrate != "provider-instance":
            errors.append(f"Device '{dev_id}': cloud-vm must use substrate 'provider-instance'")

        if dev_type != "cloud-vm" and dev_substrate == "provider-instance":
            errors.append(f"Device '{dev_id}': substrate 'provider-instance' is reserved for cloud-vm")

        if dev_substrate == "provider-instance" and dev_access == "local-lan":
            warnings.append(f"Device '{dev_id}': provider-instance usually should not use access 'local-lan'")

        if dev_substrate == "provider-instance" and dev_access not in {"public", "vpn-only"}:
            warnings.append(f"Device '{dev_id}': provider-instance access is usually 'public' or 'vpn-only'")

        if dev_substrate in {"baremetal-owned", "baremetal-colo"} and location_type == "cloud":
            warnings.append(f"Device '{dev_id}': baremetal substrate mapped to cloud location '{location_ref}'")
    ...
    for policy in l7_policies:
        ...
        if policy_device_ref and policy_device_ref not in ids["devices"]:
            errors.append(f"Power policy '{policy_id}': device_ref '{policy_device_ref}' does not exist")
        ...
        if isinstance(connection, dict):
            ...
            if connected_to and connected_to not in ids["devices"]:
                errors.append(f"Power policy '{policy_id}': connection.connected_to '{connected_to}' does not exist")
