...
if [ ! -f "topology-tools/validate-topology.py" ]; then
    print_warning "validate-topology.py not found, skipping validation"
else
    echo ""
    echo "Running validation..."
    if python3 topology-tools/validate-topology.py; then
...
if [ ! -f "topology-tools/generate-ansible-inventory.py" ]; then
    ...
    if python3 topology-tools/generate-ansible-inventory.py --output generated/ansible; then
...
if [ ! -f "topology-tools/generate-docs.py" ]; then
    print_warning "generate-docs.py not found, skipping"
else
    echo ""
    echo "Generating documentation..."
    if python3 topology-tools/generate-docs.py --output generated/docs; then
...
if git diff --quiet generated/; then
    ...
    echo "Commit these changes with: git add generated/ && git commit"
fi
...
if [ $TESTS_FAILED -eq 0 ]; then
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Review changes: git diff generated/"
    echo "  2. Commit if needed: git add generated/ && git commit -m 'Regenerate infrastructure'"
    echo "  3. Apply with Terraform: cd terraform && terraform plan"
    echo ""
    exit 0
else
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
