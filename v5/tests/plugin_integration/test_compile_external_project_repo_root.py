...
GENERATE_LOCK_SCRIPT = REPO_ROOT / "v5" / "topology-tools" / "generate-framework-lock.py"
...
def test_compile_with_external_project_repo_root(tmp_path: Path) -> None:
    ...
    error_catalog = project_repo / "v5" / "topology-tools" / "data" / "error-catalog.yaml"
    ...
    _write_yaml(
        topology_path,
        {
            ...
            "framework": {
                ...
                "class_modules_root": str(framework_root / "v5" / "topology" / "class-modules"),
                "object_modules_root": str(framework_root / "v5" / "topology" / "object-modules"),
                "model_lock": str(framework_root / "v5" / "topology" / "model.lock.yaml"),
                "profile_map": str(framework_root / "v5" / "topology" / "profile-map.yaml"),
                "layer_contract": str(framework_root / "v5" / "topology" / "layer-contract.yaml"),
                "capability_catalog": str(
                    framework_root / "v5" / "topology" / "class-modules" / "router" / "capability-catalog.yaml"
                ),
                "capability_packs": str(
                    framework_root / "v5" / "topology" / "class-modules" / "router" / "capability-packs.yaml"
                ),
            },
            "project": {
                "active": "home-lab",
    ...
    generated = subprocess.run(
        [
            ...
            str(framework_root),
            "--framework-manifest",
            str(framework_root / "v5" / "topology" / "framework.yaml"),
            "--lock-file",
    ...
    compiled = subprocess.run(
        [
            ...
            str(error_catalog),
            "--plugins-manifest",
            str(REPO_ROOT / "v5" / "topology-tools" / "plugins" / "plugins.yaml"),
            "--secrets-mode",
            "passthrough",
            "--strict-model-lock",
            "--output-json",
