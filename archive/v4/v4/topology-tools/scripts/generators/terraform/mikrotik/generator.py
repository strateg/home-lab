...
class MikrotikTerraformGenerator(TerraformGeneratorBase):
        ...
        for network in self.topology["L2_network"].get("networks", []):
            if network.get("managed_by_ref") == "mikrotik-chateau":
                ...
                if item.get("vlan"):
                    ...
                    item["interface_name"] = self._resolve_interface_name(
                        item.get("interface_ref"), default_name="bridge-lan"
                    )
                else:
    ...
    def run_terraform_fmt(self) -> bool:
        ...
        try:
            result = subprocess.run(["terraform", "fmt"], cwd=str(self.output_dir), capture_output=True, text=True)
...
"""
    ...
    def print_summary(self) -> None:
        ...
        print("  5. Run: cd deploy && make assemble-native")

        print(f"\nNext steps:")
        print("  1. cd .work/native/terraform/mikrotik")
