...
"@
    ...
    Write-Host "  1. ./scripts/secrets/unlock-secrets.ps1"
    Write-Host "  2. Re-encrypt secrets (PowerShell): Get-ChildItem projects/home-lab/secrets -Recurse -Filter *.yaml | ForEach-Object { sops updatekeys -y $_.FullName }"
} finally {
    if (Test-Path $tmpDev) {
