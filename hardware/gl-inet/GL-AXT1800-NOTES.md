...
**Ethernet:** 2x Gigabit (1 WAN + 1 LAN) ⚠️ Компактная модель!
**USB:** 1x USB 3.0
**Прошивка:** GL.iNet firmware на базе OpenWRT 21.02+
**Форм-фактор:** Компактный travel router (портативный)

> 💡 **Особенность:** Slate AX - это **компактная travel-версия** с меньшим количеством LAN портов (1 вместо 4), но с теми же WiFi 6 возможностями.
...
│  ┌───────────────────┐  │
│  │ WAN (синий порт)  │  │ ← Подключить к OPNsense дома или Hotel WiFi
│  │ LAN (желтый порт) │  │ ← Ваше устройство (только 1 порт!)
│  │ USB 3.0   POWER   │  │
│  │ RESET             │  │
│  └───────────────────┘  │
└─────────────────────────┘
...
> ⚠️ **Важно:** У Slate AX всего **1 LAN порт**! Для подключения нескольких устройств используйте WiFi или внешний USB Ethernet адаптер.
...
- `lan` - физический LAN порт (желтый) ⚠️ Только один!
...
cat /sys/class/net/lan/operstate
```
...
> 💡 **Вывод:** Slate AX (GL-AXT1800) - идеальный **travel router** для поездок, но дома может не хватать LAN портов. Используйте WiFi или USB Ethernet адаптеры.
