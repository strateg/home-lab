...
generate_stage:
  ...
  - 380-389: initialization_manifest_generator # NEW
```
...
Create `scripts/orchestration/init-node.py` as part of the deploy domain (separate from v5 pipeline):
...
python scripts/orchestration/init-node.py --node rtr-mikrotik-chateau

# Initialize all pending nodes
python scripts/orchestration/init-node.py --all-pending

# Verify handover only (no bootstrap)
python scripts/orchestration/init-node.py --verify-only --node hv-proxmox-xps
```
