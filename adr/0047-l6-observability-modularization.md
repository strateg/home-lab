...
**Updated:** 2026-02-28
**Status:** Deferred
**Trigger:** Implement when alerts > 50 OR services > 30
...
```
L6-observability/
├── alerts.yaml              # 11 alerts, flat list
├── healthchecks.yaml
├── dashboard.yaml
...
**Naming:** Currently flat (e.g., `alert-router-down`, `alert-disk-full`).

**Decision:** No changes needed at current scale.

---

## Future Design (When Triggered)

### Phase 2: Naming Convention (alerts 50-100)

**Trigger:** Alert count exceeds 50.

**Action:** Rename alerts to hierarchical naming per ADR 0048:
```
alert-{domain}.{service}-{type}

Examples:
  alert-infra.router-down
  alert-web.nextcloud-disk-full
  alert-db.postgres-replication-lag
```

**Implementation:**
- Add naming validator to enforce pattern
- Migrate existing alerts (automated script)
...
**No action required.** Current flat structure is appropriate for ~10 services and ~10 alerts.
