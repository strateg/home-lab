...
**Status note:** This plan is no longer fully deferred: scaffold work for Phase 5.3 started (`init_node` CLI/state baseline), Phase 0 environment precheck is wired into `init-node`, non-plan execution now runs adapter preflight/execute lifecycle scaffold with state transitions (`pending/failed -> bootstrapping -> initialized|failed`), mechanism-specific adapter baselines (`netinstall`, `unattended_install`, `cloud_init`, `ansible_bootstrap`) are wired via `get_adapter()` with preflight checks, `--verify-only` baseline now executes adapter handover checks and promotes `initialized -> verified`, runner workspace staging/cleanup baseline is now wired through `get_runner().stage_bundle()/cleanup_workspace()` for execute/verify paths, structured logging baseline is started (`scripts/orchestration/deploy/logging.py`, JSONL audit under `.work/deploy-state/<project>/logs/`), Taskfile execution wrappers baseline is started (`deploy:init-node-run`, `deploy:init-all-pending-run`), bundle manifest mechanism inference supports root-level bootstrap layouts (for example Proxmox `answer.toml`, Orange Pi `user-data`/`meta-data`), unknown node targets fail fast (`node-not-found`), Phase 1 schema/validator baseline is in place, Phase 2.1/2.2 baseline is started (MikroTik contract + contract-aware bootstrap projection routing), Phase 3.1/3.2/3.3/3.4 baseline is started (Proxmox contract + minimal templates + generator update), and Phase 4.1 baseline is started (Orange Pi contract declaration).
...
- `5.11` scaffold started: structured console+JSONL audit logging (`logging.py`) wired into `init-node` execution/verify/status flows.
- `5.12` baseline started: Taskfile wrappers for non-plan node/all-pending execution.
...
| 5.12 | Add Taskfile targets | `taskfiles/deploy.yml` | `task deploy:init:*` commands |
