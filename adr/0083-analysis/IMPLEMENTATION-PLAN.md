...
**Status note:** This plan is no longer fully deferred: scaffold work for Phase 5.3 started (`init_node` CLI/state baseline), Phase 0 environment precheck is wired into `init-node`, non-plan execution now runs adapter preflight/execute lifecycle scaffold with state transitions (`pending/failed -> bootstrapping -> initialized|failed`), unknown node targets fail fast (`node-not-found`), Phase 1 schema/validator baseline is in place, Phase 2.1/2.2 baseline is started (MikroTik contract + contract-aware bootstrap projection routing), Phase 3.1/3.2/3.3/3.4 baseline is started (Proxmox contract + minimal templates + generator update), and Phase 4.1 baseline is started (Orange Pi contract declaration).
