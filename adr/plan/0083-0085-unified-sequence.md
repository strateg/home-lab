...
| 0083 | Proposed (Scaffold started) | `init-node` CLI/state/status + adapter/state-machine scaffold + environment precheck + non-plan adapter preflight/execute lifecycle baseline + Phase 1 schema/validator baseline + MikroTik/Proxmox/OrangePi contract declarations + contract-aware bootstrap projection routing + Proxmox minimal bootstrap templates/generator update + tests | Complete concrete adapters/handover flow |
