...
| 0083 | Proposed (Scaffold started) | `init-node` CLI/state/status + adapter/state-machine scaffold + environment precheck + Phase 1 schema/validator baseline + MikroTik/Proxmox/OrangePi contract declarations + contract-aware bootstrap projection routing + tests | Complete concrete adapters/handover flow |
