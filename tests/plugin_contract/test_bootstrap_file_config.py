...
def test_mikrotik_bootstrap_generator_has_no_hardcoded_file_mappings() -> None:
    ...
    assert 'node_root / "init-terraform.rsc"' not in body, "File mappings must not be hardcoded in generator code"
    assert 'node_root / "README.md"' not in body, "File mappings must not be hardcoded in generator code"
    assert '"bootstrap/init-terraform.rsc.j2"' not in body, "Template paths must not be hardcoded in generator code"
...
def test_proxmox_bootstrap_generator_has_no_hardcoded_file_mappings() -> None:
    ...
    assert "script_actions = {" not in body, "Script actions must not be hardcoded in generator code"
    assert '"01-install-terraform.sh"' not in body, "Script names must not be hardcoded in generator code"
    assert 'node_root / "answer.toml.example"' not in body, "File mappings must not be hardcoded in generator code"
...
def test_orangepi_bootstrap_generator_has_no_hardcoded_file_mappings() -> None:
    ...
    assert 'cloud_init_root / "user-data.example"' not in body, "File mappings must not be hardcoded in generator code"
    assert 'cloud_init_root / "meta-data"' not in body, "File mappings must not be hardcoded in generator code"
    assert '"bootstrap/user-data.example.j2"' not in body, "Template paths must not be hardcoded in generator code"
