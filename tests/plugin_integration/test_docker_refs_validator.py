...
def test_docker_refs_manifest_requires_normalized_rows() -> None:
    registry = _registry()
...
def test_docker_refs_execute_stage_requires_committed_normalized_rows(tmp_path: Path) -> None:
    ...
    payload = {
        ...
        "plugins": [
            ...
            },
            {
                "id": PLUGIN_ID,
                "kind": "validator_json",
                "entry": f"{(V5_TOOLS / 'plugins/validators/docker_refs_validator.py').as_posix()}:DockerRefsValidator",
                "api_version": "1.x",
                "stages": ["validate"],
                "phase": "run",
                "order": 144,
