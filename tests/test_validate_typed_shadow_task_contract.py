#!/usr/bin/env python3
"""Task contract checks for typed-shadow validation wiring."""

from __future__ import annotations

from pathlib import Path
from typing import Any

import yaml


REPO_ROOT = Path(__file__).resolve().parents[1]
TASKFILE_PATH = REPO_ROOT / "taskfiles" / "validate.yml"


def _load_yaml(path: Path) -> dict[str, Any]:
    payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    assert isinstance(payload, dict), f"expected YAML mapping at {path}"
    return payload


def _first_cmd(tasks: dict[str, Any], name: str) -> str:
    row = tasks.get(name)
    assert isinstance(row, dict), f"missing task: {name}"
    cmds = row.get("cmds", [])
    assert isinstance(cmds, list) and cmds, f"missing cmds for task: {name}"
    assert isinstance(cmds[0], str)
    return cmds[0]


def test_validate_taskfile_includes_typed_shadow_tasks() -> None:
    payload = _load_yaml(TASKFILE_PATH)
    tasks = payload.get("tasks", {})
    assert isinstance(tasks, dict)
    for name in ("typed-shadow-report", "typed-shadow-gate"):
        assert name in tasks


def test_typed_shadow_task_wiring_uses_generator_and_diagnostics_artifacts() -> None:
    payload = _load_yaml(TASKFILE_PATH)
    tasks = payload.get("tasks", {})
    assert isinstance(tasks, dict)

    report_cmd = _first_cmd(tasks, "typed-shadow-report")
    gate_cmd = _first_cmd(tasks, "typed-shadow-gate")

    for cmd in (report_cmd, gate_cmd):
        assert "scripts/inspection/generate_typed_shadow_report.py" in cmd
        assert "--json-output build/diagnostics/typed-shadow-report.json" in cmd
        assert "--text-output build/diagnostics/typed-shadow-report.txt" in cmd

    assert "--fail-on-threshold" not in report_cmd
    assert "--fail-on-threshold" in gate_cmd
