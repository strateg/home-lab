...
class TerraformProxmoxGenerator(BaseGenerator):
    ...
    def execute(self, ctx: PluginContext, stage: Stage) -> PluginResult:
        ...
        projections = load_object_projection_module("proxmox", ctx=ctx)
        projection_error = projections.ProjectionError
        build_proxmox_projection = projections.build_proxmox_projection
            ...
            for message in contract_validation_errors:
                diagnostics.append(
                    self.emit_diagnostic(
                        ...
                        stage=stage,
                        message=message,
                        path="generator:terraform_proxmox:artifact_contract",
                    )
                )
            return self.make_result(diagnostics=diagnostics)
        ...
        ctx.publish("terraform_proxmox_files", written)
