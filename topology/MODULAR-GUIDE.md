...
`runtime.type: baremetal` is derived from `host_type` and does not require an explicit `baremetal` capability.
Allowed host types for native runtime are: `baremetal`, `embedded`, `hypervisor`.
